...
def update_roster_member(character_id, roster_pk):
    ...
    if response.status_code == 200:
        ...
        monthly_esi_fleet_count = EsiFleetMember.objects.filter(character_id=character_id, fleet__start_time__gte=timezone.now() - timezone.timedelta(days=30)).count()
        quarterly_esi_fleet_count = EsiFleetMember.objects.filter(character_id=character_id, fleet__start_time__gte=timezone.now() - timezone.timedelta(days=90)).count()
